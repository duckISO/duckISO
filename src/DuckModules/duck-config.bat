...
call C:\Windows\DuckModules\refreshenv.bat
...
if %postinstall%==1 (
	%currentuser% C:\Windows\DuckModules\duck-config.bat /defenderDPost /skipElevationCheck
	goto tweaksfinish
)
choice /c:yn /n /m "Would you like to disable Defender? [Y/N]"
if %errorlevel%==1 %currentuser% C:\Windows\DuckModules\duck-config.bat /defenderDPost /skipElevationCheck
